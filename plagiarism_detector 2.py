...
import tokenize
import io
import keyword  # Added to check for Python keywords
...
# Enhanced Rabin-Karp with improved token normalization
def normalize_code(code: str, filepath: str) -> str:
    # Tokenize the code
    tokens = []
    is_python = filepath.endswith('.py')
    try:
        file_obj = io.StringIO(code)
        for tok_type, tok_string, _, _, _ in tokenize.generate_tokens(file_obj.readline):
            if tok_type == tokenize.NAME:  # Variable, function name, or keyword
                if is_python and keyword.iskeyword(tok_string):  # Check if it's a Python keyword
                    tokens.append(tok_string)
                else:
                    tokens.append('<NAME>')
            elif tok_type == tokenize.OP:  # Operators
                tokens.append(tok_string)
        ...
        return ''.join(tokens)  # Join without extra spaces to avoid k-gram mismatches
    except tokenize.TokenError:
        return code  # Fallback to original code if tokenization fails

def rabin_karp(text1: str, text2: str, filepath1: str, filepath2: str, k: int = 3, q: int = 101) -> int:
    if not text1 or not text2:
        return 0

    # Normalize the code
    norm_text1 = normalize_code(text1.lower(), filepath1)
    norm_text2 = normalize_code(text2.lower(), filepath2)
    ...
    # Create k-grams
    kgrams1 = [norm_text1[i:i+k] for i in range(len(norm_text1) - k + 1)]
    kgrams2 = [norm_text2[i:i+k] for i in range(len(norm_text2) - k + 1)]

    # Compute hash for first k-gram of text1
    p = 0
    for i in range(k):
        p = (d * p + ord(norm_text1[i])) % q

    # Store hashes for text2 k-grams
    t_hashes = set()
    t = 0
    for i in range(k):
        t = (d * t + ord(norm_text2[i])) % q
    t_hashes.add(t)

    for i in range(len(norm_text2) - k):
        t = (d * (t - ord(norm_text2[i]) * h) + ord(norm_text2[i + k])) % q
        if t < 0:
            t += q
        t_hashes.add(t)
    ...
    for i in range(len(kgrams1)):
        if p in t_hashes:
            ...
            if kgrams1[i] in kgrams2:
                matches += 1
        if i < len(kgrams1) - 1:
            p = (d * (p - ord(norm_text1[i]) * h) + ord(norm_text1[i + k])) % q
            if p < 0:
                p += q
...
class PlagiarismDetector:
    ...
    def compute_similarities(self):
        for i, (id1, content1) in enumerate(self.files.items()):
            meta1 = self.metadata.search(id1)
            filepath1 = meta1['filepath'] if meta1 else ''
            for id2, content2 in list(self.files.items())[i+1:]:
                meta2 = self.metadata.search(id2)
                filepath2 = meta2['filepath'] if meta2 else ''
                # Skip comparison if files are of different languages
                if filepath1.endswith('.py') != filepath2.endswith('.py'):
                    continue
                score = rabin_karp(content1, content2, filepath1, filepath2, k=self.k)
    ...
    def add_file(self, filepath: str, metadata: dict) -> dict:
        ...
        for id2, content2 in self.files.items():
            if id2 != file_id:
                meta2 = self.metadata.search(id2)
                filepath2 = meta2['filepath'] if meta2 else ''
                score = rabin_karp(self.files[file_id], content2, filepath, filepath2, k=self.k)
...
if __name__ == "__main__":
    ...
    detector = PlagiarismDetector(similarity_threshold=50.0, k=2)  # Lowered k to 2 for finer-grained matching
    files = {
    ...
    detector = PlagiarismDetector(similarity_threshold=50.0, k=2)
    edge_files = {
